...
class Knob extends HTMLElement {
    ...
    connectedCallback() {
        ...
        new Dragging(this, {
            ...
            move: (drag) => {
                ...
                location.isIndex && this.hasAttribute('alt') && Math.abs(this.θ() - Dragging.start) >= (this.maxθ - this.minθ)/2
                    && (this.Q('a').href = this.getAttribute('alt')) && this.shadowRoot.Q('div').classList.add('dragged');
                location.isIndex && this.hover('remove');
    ...
    beforeChildren() {
        location.isIndex && this.hover('add');
